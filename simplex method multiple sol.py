...
def print_table(a_prime, c_prime, d_prime, b_prime, c, list_of_basic_variables, list_of_non_basic_variables, x, z):
    l_ = []
    for i in range(1, len(list_of_basic_variables)+1):
        if list_of_basic_variables[i][0] == 'z':
            l_.append(list_of_basic_variables[i][0] + str(i) + ' = ' + str(z[int(list_of_basic_variables[i][1])-1]))
        if list_of_basic_variables[i][0] == 'x':
            l_.append(list_of_basic_variables[i][0] + str(i) + ' = ' + str(x[int(list_of_basic_variables[i][1])-1]))
    print("B = ", l_)
    l_ = []
    for i in range(1, len(list_of_non_basic_variables)+1):
        if list_of_non_basic_variables[i][0] == 'z':
            l_.append(list_of_non_basic_variables[i] + ' = ' + str(z[int(list_of_non_basic_variables[i][1])-1]))
        if list_of_non_basic_variables[i][0] == 'x':
            l_.append(list_of_non_basic_variables[i] + ' = ' + str(x[int(list_of_non_basic_variables[i][1])-1]))
    print("N = ", l_)
    print(a_prime)
    print(b_prime)
    print(-c_prime)
...
def simplex(n_prime, m_prime, a_prime, c_prime, d_prime, b_prime):
    ...
    while True:
        ...
        print("-" * 60)
        print("Iteration ", ite)
        print_table(a_prime, c_prime, d_prime, b_prime, c, list_of_basic_variables, list_of_non_basic_variables, x, z)
        v = np.argmax(c_prime)
        cv = c_prime[v]
        if cv < 0:
            print("-" * 60)
