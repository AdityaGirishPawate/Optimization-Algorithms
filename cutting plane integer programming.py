...
def print_standard_form(n, m, a, c, d, b, s):
    ...
    for i in range(len(list_of_nonbasic_variables)):
        cn[i] = c[i]
    # c = np.empty((len(all_variables)))
    c = np.empty((len(list_of_nonbasic_variables)))
    # a = np.concatenate([a, np.array(add_col)], axis=1)
    for i in range(len(list_of_nonbasic_variables)):
        c[i] = 0
...
"""def simplex(a, b, cb, cn, c, x, list_of_nonbasic_variables, list_of_basic_variables, all_variables, list_of_artificial_variables, list_of_slack_variables, list_of_surplus_variables):

    m_prime = len(list_of_basic_variables)
    n_prime = len(list_of_nonbasic_variables)
    ite=0
    while True:
        ite += 1
        print("-" * 60)
        print("Iteration ", ite)
        print("The value of Objective function in this iteration is ", round(val_objective_function(cb, b, d, optimization), 5))
        ...
        ratios = np.empty((len(list_of_basic_variables)))
        min_ratio = INFINITY
        u = -1
        pivot = -1
        for i in range(len(b)):
        ...
        if min_ratio == INFINITY:
            print("-" * 60)
            print("The problem is unbounded in the simplex method")
            return a, b, cb, cn, c, x, list_of_nonbasic_variables, list_of_basic_variables, all_variables, list_of_slack_variables
        ...
        for j in range(n_prime):
            if j == v:
                ...
                c_new[j] = (pivot * c[j] - c[v] * a[u][j]) / pivot
        b_new = np.copy(b)
